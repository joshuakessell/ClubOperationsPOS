...
const LEGACY_ALLOWED_PATH_PREFIXES = ['docs/database/', 'services/api/migrations/', 'db/'].map(
  (p) => p.replaceAll('\\', '/')
);
  ...
  try {
    ...
    for (const forbidden of FORBIDDEN_STRINGS) {
      if (enumBody.includes(forbidden)) {
        errors.push(
          `❌ packages/shared/src/enums.ts: RoomType enum contains forbidden value: ${forbidden}`
        );
      }
    }
    ...
    for (const required of REQUIRED_TIER_VALUES) {
      if (!enumBody.includes(`${required} = '${required}'`)) {
        errors.push(
          `❌ packages/shared/src/enums.ts: RoomType enum missing required value: ${required}`
        );
      }
    }
    ...
    if (!enumBody.includes("LOCKER = 'LOCKER'")) {
      warnings.push(
        `⚠️  packages/shared/src/enums.ts: RoomType enum missing LOCKER (expected but not required)`
      );
    }
...
function checkForbiddenStrings() {
  ...
  console.log(
    `   - Allowing legacy strings (not scanned): ${LEGACY_ALLOWED_PATH_PREFIXES.join(', ')}`
  );
  console.log('');

  const filesToCheck = SOURCE_ROOTS_TO_SCAN.flatMap((root) =>
    findFiles(root, ['ts', 'tsx', 'js', 'jsx'])
  ).filter((f) => {
    ...
    try {
      ...
      for (const forbidden of FORBIDDEN_STRINGS) {
        ...
        if (lineNumbers.length > 0) {
          errors.push(
            `❌ ${relPath}: Found forbidden string "${forbidden}" (lines: ${lineNumbers.join(', ')})`
          );
          foundForbidden = true;
        }
      }
  ...
  try {
    ...
    for (const forbidden of ['VIP', 'Deluxe']) {
      if (
        ...
        !content.includes(`Note: ${forbidden}`) &&
        !content.includes(`old ${forbidden}`)
      ) {
        warnings.push(
          `⚠️  SPEC.md: Contains "${forbidden}" - ensure this is only in historical/context notes`
        );
      }
    }
