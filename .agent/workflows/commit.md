...
4. Run database migrations and seed the local demo database:
   ```bash
   cd /Users/joshuakessell/Projects/ClubOperationsPOS && DB_HOST=localhost DB_PORT=5433 DB_PASSWORD=club-ops-dev pnpm --filter @club-ops/api run db:migrate && DB_HOST=localhost DB_PORT=5433 DB_PASSWORD=club-ops-dev pnpm --filter @club-ops/api run seed:all
   ```
