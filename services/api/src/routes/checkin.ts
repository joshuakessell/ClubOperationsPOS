        ...
        const result = await transaction(async (client) => {
          ...
          if (checkinMode === 'RENEWAL' && visitId) {
            ...
            const visitResult = await client.query<{
              ...
              customer_id: string;
              started_at: Date;
            }>(`SELECT id, customer_id, started_at FROM visits WHERE id = $1`, [visitId]);
        ...
        const result = await transaction(async (client) => {
          ...
          if (idScanHash) {
            ...
            const existingCustomer = await client.query<{
              ...
              name: string;
              dob: Date | null;
            }>(`SELECT id, name, dob FROM customers WHERE id_scan_hash = $1 LIMIT 1`, [idScanHash]);
