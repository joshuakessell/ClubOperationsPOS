...
export function CustomerAccountPanel(props: {
  ...
  }, [props.customerId, props.sessionToken, customerNotesState, customerSpendLedgerState]);
  ...
  const fallbackName =
    profile?.name || props.customerSummary?.name || props.customerLabel || '\u2014';
  const fallbackDob = profile?.dobMonthDay ?? props.customerSummary?.dobMonthDay ?? null;
  const fallbackMembership =
    profile?.membershipNumber ?? props.customerSummary?.membershipNumber ?? null;
  ...
  const showGoBack =
    state.mode === 'ERROR' && (state.errorCode === 'UNDERAGE' || state.errorCode === 'ID_EXPIRED');
