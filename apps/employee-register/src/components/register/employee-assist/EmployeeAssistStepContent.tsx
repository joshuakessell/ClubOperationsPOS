...
export function EmployeeAssistStepContent({
  ...
  const [requestedResourceType, setRequestedResourceType] = useState<'room' | 'locker' | null>(
    null
  );
    ...
    return (
      <div style={{ display: 'grid', gap: '0.75rem' }}>
          ...
          >
            ...
            >
              <div
                ...
                style={{ color: '#94a3b8', fontWeight: 900, marginBottom: '0.45rem' }}
              >
                Stand - by room types
              </div>
            ...
            >
              ...
              <select
                className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-800 shadow-theme-xs outline-none transition focus:border-brand-300 focus:ring focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                value={
        ...
        <div style={{ display: 'grid', gap: '0.6rem' }}>
          {waitlistBackupButtons
            .filter((btn) => btn.allowed && btn.count > 0)
            .map((btn) => {
              ...
              const toneClass =
                tone === 'none'
                  ...
                  : tone === 'low'
                    ? 'border border-warning-300 bg-warning-50 text-warning-600 dark:border-warning-700 dark:bg-warning-500/10 dark:text-warning-400'
                    : 'border border-gray-200 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]';
          ...
          <button
            ...
            className={[
              ...
              pending?.step === 'WAITLIST_JOIN' && pending.option === unavailableJoinTarget
                ? 'border-2 border-brand-500 bg-brand-50 font-semibold text-brand-600 dark:bg-brand-500/10 dark:text-brand-400'
                : 'border border-gray-200 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]',
            ].join(' ')}
            disabled={isSubmitting}
            onClick={() => {
      ...
      <div style={{ display: 'grid', gap: '0.75rem' }}>
        ...
        <div
          ...
          style={{
            ...
            background: paid ? 'rgba(34, 197, 94, 0.08)' : 'rgba(251, 191, 36, 0.08)',
          }}
        >
  ...
  if (step === 'AGREEMENT') {
    ...
    const availableRoomsList: Array<{
      type: 'room' | 'locker';
      tier: string;
      number: string;
      count: number;
    ...
    if (inventoryAvailable) {
      ...
      if (selectedType === 'LOCKER' || selectedType === 'GYM_LOCKER') {
        ...
        const tiers = selectedType ? [selectedType] : ['STANDARD', 'DOUBLE', 'SPECIAL'];
    ...
    return (
      <div style={{ display: 'grid', gap: '0.75rem' }}>
        ...
        {onBypassAgreement ? (
          <button
            type="button"
            className={`inline-flex items-center justify-center rounded-lg px-4 py-2.5 text-sm font-semibold shadow-theme-xs transition ${
              agreementBypassPending
                ? 'border-brand-500 bg-brand-50 font-semibold text-brand-600 dark:bg-brand-500/10 dark:text-brand-400'
                : 'border border-gray-200 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]'
            }`}
        ...
        {availableRoomsList.length > 0 ? (
          ...
          >
            ...
            <div
              className="er-text-sm"
              style={{ color: '#94a3b8', fontWeight: 700, marginBottom: '0.5rem' }}
            >
            ...
            <div style={{ display: 'grid', gap: '0.4rem' }}>
              {availableRoomsList.map((item) => (
                ...
                >
                  ...
                  <span className="er-text-sm" style={{ color: '#94a3b8', fontWeight: 900 }}>
